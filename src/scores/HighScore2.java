...
public class HighScore2 {
	 ...
	 */
	public ArrayList<BestPlayer2> tenBestScores(ArrayList<String[]> scoreUnranked)
	{
		ArrayList<BestPlayer2> scoreRanked = new ArrayList<BestPlayer2>();
		int compt=0;
		...
		while(scoreUnranked.size()>0 && compt<10)
		{
			...
			BestPlayer2 bp = new BestPlayer2(result[1],Integer.parseInt(result[0]));
			
			//On récupère le couple score,player avec le score le plus grand
			while (itr.hasNext()){
				try{
					result =(String[]) itr.next();
					BestPlayer2 p = new BestPlayer2(result[1],Integer.parseInt(result[0]));
					if(p.compareTo(bp)==1)
					{
						bp=p;
						cplMax=result;
